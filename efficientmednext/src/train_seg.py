#!/usr/bin/env python3
"""
Stage 1: Pure ICH Segmentation DDP Training (nnU-Net v2 style pipeline)
"""
import os
import sys
import time
...
def parse_args():
    parser = argparse.ArgumentParser(description='Stage 1: ICH Segmentation Training')
    ...
    parser.add_argument('--patch_size', type=int, nargs=3, default=[128, 128, 16])
...
def main():
    ...
    if is_main_process():
        ...
        logger = setup_logger('train_seg', os.path.join(args.output_dir, 'train.log'))
    ...
    optimizer = torch.optim.SGD(model.parameters(), lr=args.lr, momentum=0.99, weight_decay=3e-5, nesterov=True)
    scheduler = LambdaLR(optimizer, lr_lambda=lambda ep: (1.0 - ep / args.epochs) ** 0.9)
    scaler = GradScaler()
    ...
    for epoch in range(args.epochs):
        ...
        if is_main_process():
            ...
            logger.info(f"Epoch {epoch} loss: {t_loss:.4f} | val loss: {v_loss:.4f} | Dice: {pseudo_dice:.4f} | EMA: {ema_dice:.4f}")

            if (epoch + 1) % args.save_every == 0 and epoch != args.epochs - 1:
                save_checkpoint(model, optimizer, scaler, best_ema, history, epoch + 1, os.path.join(args.output_dir, 'checkpoint_latest.pth'))

            if best_ema is None or ema_dice > best_ema:
                best_ema = ema_dice
                logger.info(f"New best EMA: {best_ema:.4f}")
                save_checkpoint(model, optimizer, scaler, best_ema, history, epoch + 1, os.path.join(args.output_dir, 'checkpoint_best.pth'))
    ...
    if is_main_process():
        save_checkpoint(model, optimizer, scaler, best_ema, history, args.epochs, os.path.join(args.output_dir, 'checkpoint_final.pth'))
